...
$aliases[$foobar[$axis]['table_id']] = $foobar[$axis]['alias'];
			...
			foreach ($method_steps as $step) {
				...
				if (!$intermediate_step) {
					...
					$prev_step_start_column_id = $prev_step_start;

					$prev_step_start_table_id = $columns[$prev_step_start_column_id]['table_id'];

					if ($step_a_table_id == $prev_step_start_table_id) {
						$step_start = $step_a;
						$step_end = $step_b;
					} else if ($step_b_table_id == $prev_step_start_table_id) {
						$step_start = $step_b;
						$step_end = $step_a;
					}
				}
...
if (empty($aliases[$step_start_table_id])) {
	$aliases[$step_start_table_id] = "j".$foobar[$axis]['alias'].rand(0,99);
}
if (empty($aliases[$step_end_table_id])) {
	$aliases[$step_end_table_id] = "j".$foobar[$axis]['alias'].rand(0,99);
}
