...
function process_html($html) {
  ...
  $temphtml = preg_replace("#.*({$open})#is","\\1",$html);
  if($temphtml!=null) { $html = $temphtml; }

  // Cut off all data before and after 'html' tag; unless we'll do it,
  // the XML parser will die violently
  $temphtml = preg_replace("#^.*<html#is","<html",$html);
  if($temphtml!=null) { $html = $temphtml; }

  $temphtml = preg_replace("#</html\s*>.*$#is","</html>",$html);
  if($temphtml!=null) { $html = $temphtml; }
...
function process_body($html) {
  ...
  /* this preg_replace can break sometimes, hence the $temphtml */
  $temphtml = preg_replace("#({$chead})(.+)({$open})#is","\\1\\3\\2",$html);
  if($temphtml!=null){$html=$temphtml;}
...
function html2xhtml($html) {
  ...
  $html = process_html($html); /* This function returns an empty string, breaking the report, for report 551, in which a string is passed as user input */
