...
// 		$admin_tools[] = array("template/default_access", "Default Report Access - New Report");
	...
	function hook_top_menu() {
		return array(
			"reports" => "<a href='".$this->webroot()."template/home'>Reports</a>"
			);
	}
...
// 		echo $q."<br/>";
	...
	function join($tables, $optional=array()) {
		if (true) {
			$foobar = $tables;
		}
		...
		$axis_keys = array_combine(array_keys($tables), array_keys($tables));
		unset($axis_keys['c']);
		...
		foreach ($axis_keys as $axis) {
			...
			if (count($method) > 2) die;

			$method_start = (int)$method[0];
			$method_end = (int)$method[1];

			$columns_query = $this->dobj->db_fetch_all($this->dobj->db_query("SELECT * FROM columns WHERE column_id='".$method_start."' OR column_id='".$method_end."';"));
			foreach ($columns_query as $column_tmp) {
				$columns[$column_tmp['column_id']] = $column_tmp;
			}

			$method_start_column = $columns[$method_start]['name'];
			$method_end_column = $columns[$method_end]['name'];

			$method_start_table = $columns[$method_start]['table_id'];
			$method_end_table = $columns[$method_end]['table_id'];

			$alias_tmp = $foobar['c']['alias'];
			if ($method_start_table == $foobar['c']['table_id']) {
				$join_foo = "$alias_tmp.$method_start_column";
			} else if ($method_end_table == $foobar['c']['table_id']) {
				$join_foo = "$alias_tmp.$method_end_column";
			}

			$alias_tmp = $foobar[$axis]['alias'];
			if ($method_start_table == $foobar[$axis]['table_id']) {
				$join_bar = "$alias_tmp.$method_start_column";
			} else if ($method_end_table == $foobar[$axis]['table_id']) {
				$join_bar = "$alias_tmp.$method_end_column";
			}
...
// 		return $output;
	...
	function view_add($module, $objects, $type='', $modules) {
		...
		if (empty($module)) {
			...
			$output->data .= "<h3>".$this->l("template/add/list", "Create List Report", "class='disabled'")."</h3>";
			$output->data .= "<p class='h3attach'>Lorem ipsum dolor sit amet.</p>";
...
// 		$output->layout = "ajax";
// 		$output->report_title = "Report Workspace";
// 		$output->data = "<div dojoType='dojox.layout.ContentPane' layoutAlign='client' id='report_workspace'>";
...
// 		$output->data .= "</div>";
	...
	function theme_reports($report, $view_id) {
		...
		$desc = $report['description'] ? $report['description'] : "No description available.";
