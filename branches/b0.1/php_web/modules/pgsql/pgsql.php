...
class Pgsql extends Catalogue {
	...
	function hook_connect($hostname, $database, $username, $password) {
		$this->conn = pg_connect("host=".$hostname." dbname=".$database." user=".$username." password='".$password."'");
		return true;
	}
...
// 			$jname = $this->find_join2($jname);
	...
	function test_connection($data) {
		...
		$test_connection = @pg_connect("host=".$db['host']." dbname=".$db['name']." user=".$db['username']." password='".$db['password']."'");
