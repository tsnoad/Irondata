...
// 		return $saved_report;
	...
	function line_graph($template_id, $saved_report_id, $graph_data, $stacked=false, $svg=true, $pdf=false) {
		...
		//x axis title
		$svg_data .= $this->generate_x_index("line", $results, ucwords($template_axies['x']['tabular_template_human_name']), $area);

		//y axis title
		$svg_data .= $this->generate_y_index($results, ucwords($template_axies['c']['tabular_template_human_name']), $area);

		//c axis title
		$return_tmp = $this->generate_c_index("line", $results, ucwords($template_axies['y']['tabular_template_human_name']), $area);
	...
	function area_graph($template_id, $saved_report_id, $graph_data, $stacked=false, $svg=true, $pdf=false) {
		...
		//x axis title
		$svg_data .= $this->generate_x_index("area", $results, ucwords($template_axies['x']['tabular_template_human_name']), $area);

		//y axis title
		$svg_data .= $this->generate_y_index($results, ucwords($template_axies['c']['tabular_template_human_name']), $area);

		//c axis title
		$return_tmp = $this->generate_c_index("area", $results, ucwords($template_axies['y']['tabular_template_human_name']), $area);
		$svg_data .= $return_tmp[0];
		$area = $return_tmp[1];
	...
	function bar_graph($template_id, $saved_report_id, $graph_data, $stacked=false, $svg=true, $pdf=false) {
		...
		//x axis title
		$svg_data .= $this->generate_x_index("bar", $results, ucwords($template_axies['x']['tabular_template_human_name']), $area);

		//y axis title
		$svg_data .= $this->generate_y_index($results, ucwords($template_axies['c']['tabular_template_human_name']), $area);

		//c axis title
		$return_tmp = $this->generate_c_index("bar", $results, ucwords($template_axies['y']['tabular_template_human_name']), $area);
		$svg_data .= $return_tmp[0];
		$area = $return_tmp[1];
...
// print_r($results['c_index']);
// var_dump(empty($results['c_index']));
// var_dump(count($results['c_index']));
...
// 					$svg_data .= "<polyline points='$points_tmp' style='fill: none; stroke: $dash_tmp; stroke-width: 3px; stroke-dasharray: $seq_tmp;' />";
			...
			case "bar":
				...
				$zero_tmp /= ($results['max_c'] - $results['min_c']);
